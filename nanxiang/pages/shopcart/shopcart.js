...
Page({
  ...
  btn_submit_order: function () {
    ...
    //   })
    let url = '../submitOrder/submitOrder?';
    let arr = [],index = [],total;
    for (let i = 0; i < list.length; i++) {
      // 判断选中计算价格
      if (list[i].selected) {
    ...
    // console.log("url:"+url)
    wx.navigateTo({
      url: '../submitOrder/submitOrder?data=' + JSON.stringify(arr) + "&total=" + that.data.totalPrice + "&index=" + JSON.stringify(index),
    })
  },
  /**
   * 计算总价
  ...
  //       let num = res.data.data.datas.length;
  //       // console.log(num);
